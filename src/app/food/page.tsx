...
import { ArrowLeft, Search, MapPin, Star, Clock, Leaf, Filter, ChevronDown, ExternalLink } from 'lucide-react'
...
const PRICE_LABELS = { budget:'Budget (₹)', mid:'Mid-range (₹₹)', splurge:'Splurge (₹₹₹)' }
  ...
  return (
    <div className="card p-5 shadow-soft hover:shadow-card hover:-translate-y-0.5 transition-all duration-200">
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-1">
            ...
            {r.isJainFriendly && (
              <span className="px-2 py-0.5 rounded-full bg-purple-50 border border-purple-200 text-purple-700 text-xs font-bold">☯️ Jain</span>
            )}
          </div>
        ...
        <div className="text-right flex-shrink-0">
          <div className="flex items-center gap-1 justify-end mb-1">
            ...
            <span className="font-bold text-slate-900 text-sm">{r.rating}</span>
          </div>
      ...
      <div className="flex items-center gap-2 flex-wrap mb-3">
        {r.tags.map(t => (
          <span key={t} className="px-2.5 py-1 bg-slate-50 rounded-full text-xs text-slate-500 font-medium">{t}</span>
        ))}
      </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={()=>setSaved(v=>!v)}
            className={cn('px-3 py-1.5 rounded-xl text-xs font-bold border-2 transition-all',
              saved?'border-sea-400 bg-sea-50 text-sea-700':'border-slate-200 text-slate-500 hover:border-sea-300')}>
            {saved?'✓ Saved':'+ Save'}
          </button>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-gradient-to-b from-sea-50 to-white">
      ...
      <nav className="bg-white/90 backdrop-blur-xl border-b border-sea-100 sticky top-0 z-10">
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center gap-3">
          ...
          <Link href="/events" className="ml-auto btn-ghost text-xs px-3 py-1.5">✨ What&apos;s On</Link>
        </div>
      </nav>
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-4">
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2 overflow-x-auto scrollbar-hide">
            {([
              ...
              <button key={f.k} onClick={()=>setDietFilter(f.k)}
                className={cn('flex-shrink-0 px-4 py-2 rounded-full text-xs font-bold border-2 transition-all',
                  dietFilter===f.k?'border-sage-400 bg-sage-50 text-sage-700 shadow-soft':'border-slate-200 text-slate-500 hover:border-sage-300')}>
                {f.l}
              </button>
            ))}
            <button onClick={()=>setShowFilters(v=>!v)}
              ...
              <Filter size={11}/> More filters <ChevronDown size={11} className={cn('transition-transform', showFilters&&'rotate-180')}/>
            </button>
          </div>
          ...
          {showFilters && (
            <div className="card p-4 space-y-3 animate-fade-up">
              ...
              <div>
                <p className="text-xs font-bold text-slate-500 mb-2">Cuisine</p>
                <div className="flex gap-2 flex-wrap">
        ...
        <div className="flex items-center justify-between">
          <p className="text-sm text-slate-500 font-medium">
            <span className="font-extrabold text-slate-900">{filtered.length}</span> restaurants found
            {dietFilter==='veg' && <span className="ml-2 text-sage-600 font-semibold">· Veg only 🥗</span>}
            {dietFilter==='jain' && <span className="ml-2 text-purple-600 font-semibold">· Jain friendly ☯️</span>}
          </p>
        </div>
